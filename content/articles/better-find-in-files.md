...
Text is content search backed by ripgrep, Files is fuzzy filename and path search,
Symbols asks your language server for workspace symbols, and All runs the three and
returns one sectioned list.
...
The first is Windows paths. ripgrep's `--vimgrep` output is `path:line:col:text`, and
you parse it by splitting on `:`. On Windows, `C:\src\Foo.cs:42:8:...` has a colon in
the drive letter, so a naive split hands you a file called `C` on line `\src\Foo.cs`.
The parser sniffs for that case and re-joins the first two fields.

The second is the fallback. If ripgrep isn't found, or the spawn fails, or it errors mid-run, the
...
Living in that space turned up three sharp edges.
...
the panel exists to grab the current selection and seed the search box, it's already
gone.
...
And focus needs saying twice. The panel posts a `focusInput` message at 100ms and
...
Explorer highlighting came first. Files matching the current query get badged in VS
Code's Explorer tree, live, as you type. Search a symbol and the file tree quietly
tells you the shape of the answer - which folders it's in, how spread out it is -
before you read a single result.

The other is a Usages CodeLens. In Rider I click a method name to see its usages. That gesture
...
For a tool I use several hundred times a day, the payback was immediate. What
surprised me was how little of it I actually wrote. The fast search engine was already
installed. The symbol index was already running. The semantic reference data was
already there behind the language server. Nearly all of the real work was in the
seams: parsing a colon-delimited format on the one OS where colons are ambiguous, and
convincing a webview to behave like a window.
