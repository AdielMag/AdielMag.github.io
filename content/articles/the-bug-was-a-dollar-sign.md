...
and timeframe. Six new observer containers came up, BTC and SOL and ETH across
5-minute and 15-minute markets: dashboards green, charts streaming, ticks
recording.
...
**for Bitcoin at $80,000.** A $5 floor is about 0.006% of BTC's price, a sensible
"volatility is never truly zero" backstop.

Now run the same clamp for Solana at $100. The floor says SOL's volatility can
never be estimated below $5 per √second, which is **5% of the entire asset
price** per root-second. The real figure is closer to half a cent. The clamp inflated SOL's
...
What stays with me isn't the fix, it's how the failure *presented*.

A gate that never opens looks like caution. If the bug had inverted a comparison
and made the bot trade constantly, I'd have caught it in minutes. A
too-conservative bot produces no losses, no errors, and no evidence, just an
absence of trades that's indistinguishable from "the market hasn't offered an
edge yet." The eventual tell was statistical rather than visual: days of markets
where the dashboard showed obviously-safe situations the bot kept declining.

The constants also didn't look like BTC code. The feed was generic, the market
...
And every test was a BTC test. Weeks of profitable production on BTC validated
...
migration that could corrupt weeks of recorded history. It still bothers me
every time I open that file, which is probably the point.

*More from the MoneyMaker devlog - the z-score gate itself and the manipulation
armor - on the [article list](index.html#articles).*
