...
One decision shapes everything else about **ClashUp's** netcode: the phone in your
hand is not allowed to decide anything.
...
The moment the client is *allowed* to be right about anything that matters, you get
two problems that never fully go away. A player who controls the simulation controls
the outcome, so anything the client decides, a modified client can lie about. And
without one authority, every device drifts into its own slightly different version of
the match, and you spend forever trying to reconcile them.

So there's exactly one simulation, and it runs on the server. Everyone else is
watching a broadcast of it. One source of truth, and the phone isn't it.
...
The techniques that fix it come straight from Gabriel Gambetta's
[Fast-Paced Multiplayer](https://www.gabrielgambetta.com/client-server-game-architecture.html)
series, which is the reference every netcode implementation quietly leans on.

## Prediction puts you in the present
...
## Reconciliation corrects you without you noticing
...
Acking by input *number* rather than by clock time is what makes this robust. Client
and server clocks drift; a sequence counter the server echoes back is exact, so
precisely the right inputs get replayed and nothing rubber-bands.

## Everyone else renders a hair in the past
...
them a fraction of a second behind. Toggle interpolation off to see the raw
snapshots jump; on to see it smoothed.

[demo:interpolation]

**See yourself in the present, everyone else in the past.** Instant response for your
own movement, smooth motion for everyone else, at the cost of viewing them slightly
behind.

## Lag compensation makes the shot count

That trade has a sting. When you line up a shot, you're aiming at where the enemy
*was*, not where they are now. Fire, and by the time your shot reaches the server
they've already moved, so a perfectly aimed shot misses.
...
so it can reward good aim. The flip side is that occasionally you'll get hit a moment
after ducking behind cover, because on the shooter's screen you were still exposed.
...
The temptation shows up constantly, and I've argued myself into it more than once. A
client gets stuck because it missed a message from the server - say, the signal that
the match ended. The lazy fix is a client-side timeout: if I haven't heard from the
server in three seconds, just end the match myself. It works in the demo. And it's
poison.

The moment the client *synthesizes* an authoritative event, the contract is broken,
and a real server bug is now hidden behind a plausible-looking guess. So the rule
holds even when it's inconvenient. If the client is stuck waiting on the server, the
fix goes in the *server* - deliver the message reliably, replay it on reconnect -
never in a client that invents the answer.

## Why it's worth the trouble

A dumb client is more work up front. You build prediction, reconciliation,
interpolation, and lag compensation instead of just moving a sprite. What you get
back is a game that's fair by construction and honest about where its bugs actually
live, which for a competitive brawler I'm maintaining alone is the part that saves
me time later.

The phone is dumb on purpose.
